...
1) Edit `ecosystem.config.js` in the root folder to match the correct directories for log files and bot root folder.

2) Edit `default.json` in `/config`. You will use the same info in the next step.
